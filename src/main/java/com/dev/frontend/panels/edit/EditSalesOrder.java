...
public class EditSalesOrder extends EditContentPanel
{
	...
	public boolean bindToGUI(Object o) {
		...
		if(salesOrderDTO.getOrderNumber() != null && salesOrderDTO.getOrderNumber().trim().length() > 0){
			txtOrderNum.setText(salesOrderDTO.getOrderNumber());
		}
		txtTotalPrice.setText(salesOrderDTO.getTotalPrice().toString());
		if(defaultTableModel.getRowCount() == 0){
			for (OrderLineDTO orderLine : salesOrderDTO.getOrderLines()) {
				defaultTableModel.addRow(new String[] {orderLine.getProductDTO().getCode().toString(), orderLine.getQuantity().toString(), orderLine.getUnitPrice().toString(), orderLine.getTotalPrice().toString() });
			}
		}
		
		if(txtCustomer.getItemCount() == 0){
	...
	public Object guiToObject() {
		...
		salesOrderDTO.setOrderNumber(txtOrderNum.getText());
		ComboBoxItem comboBoxCustomerItem = (ComboBoxItem) txtCustomer.getSelectedItem();
		salesOrderDTO.setCustomer((CustomerDTO) Services.readRecordByCode(comboBoxCustomerItem.getKey(), Services.TYPE_CUSTOMER));
		
		salesOrderDTO.setTotalPrice(Double.parseDouble(txtTotalPrice.getText()));
		List<OrderLineDTO> orderLines = new ArrayList<OrderLineDTO>();
		for (int i = 0; i < defaultTableModel.getRowCount(); i++) {
			OrderLineDTO orderLine = new OrderLineDTO();
			ProductDTO product = (ProductDTO) Services.readRecordByCode((String) defaultTableModel.getValueAt(i, 0), Services.TYPE_PRODUCT);
			orderLine.setProductDTO(product);
			Integer quantity = Integer.valueOf(defaultTableModel.getValueAt(i, 1).toString());
			orderLine.setQuantity(quantity);
			Double unitPrice = Double.valueOf(defaultTableModel.getValueAt(i, 2).toString());
			orderLine.setUnitPrice(unitPrice);
			Double totalPrice = Double.valueOf(defaultTableModel.getValueAt(i, 3).toString());
		...
		salesOrderDTO.getOrderLines().addAll(orderLines);
